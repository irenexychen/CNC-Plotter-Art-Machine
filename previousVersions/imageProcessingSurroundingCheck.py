...
print im_resize.shape[0:1]
print im_resize.shape[1:2]
print im_resize.shape[0]
print im_resize.shape[1]
maxX = x
maxY = y
...
for i in range(im_outline.shape[1]):  		#i = current x coord
	for j in range(im_outline.shape[0]): 		#j = current y coord
		print ("COORDINATES", i, j)
		if ( 0 <= i < x and 0<= j < y and im_outline[i][j] == 0):
			print("IS SMALLER")
			if (foundX[i] == 0 or foundY[j] == 0):
