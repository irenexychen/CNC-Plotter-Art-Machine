...
def checkSurrounding(i,j):
	...
	for cX in range (-1, 2):
		for cY in range (-1, 2):
			newX = x + cX
			newY = y + cY
			print("looking at: " + str(newX) + ", " + str(newY))
			if (0 <= newX < x and 0 <= newY < y):
				if(foundX[newX] == 0 or found[newY] == 0) and (im_outline[newY][newX] == 0):
					coordsX = np.append(coordsX, newX)
					coordsY = np.append(coordsY, newY)
					print >> f, new, j
					print("found: ", i, j, cX, cY, newX, newY)
					foundX[newX] = 1
					foundY[newY] = 1
					checkSurrounding(newY, newX)
			foundX[newX] = 1
			foundY[newY] = 1
		...
		if ( 0 <= i < x and 0<= j < y):
			if (im_outline[i][j] == 0):
				if (foundX[i] == 0 or foundY[j] == 0):
					...
					coordsY = np.append(coordsY, j)				
					print >> f, i, j
					print("found in initial for-loop", i, j)
					foundX[i] = 1
					foundY[j] = 1
